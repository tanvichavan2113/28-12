...
async function calculateAndInsertRanges3() {
  ...
  try {
    ...
    await client.query(`
      CREATE TEMPORARY TABLE GroupedValues AS (
        SELECT
          timestamp,
          value,
          (ROW_NUMBER() OVER (ORDER BY timestamp) - 1) / 3 + 1 AS group_id
        FROM
          values_measurement
      );
    `);

    // Execute the second CTE to get aggregated values
    await client.query(`
      CREATE TEMPORARY TABLE AggregatedValues AS (
        SELECT
          group_id,
          MIN(value) AS first_value,
          MAX(value) AS second_value,
          LEAD(MIN(value), 2) OVER (ORDER BY MIN(timestamp)) AS third_value
        FROM
          GroupedValues
        GROUP BY
          group_id
    ...
    // Insert calculated ranges into range_chart3
    await client.query(`
      INSERT INTO range_chart3 (first, second, third, min_value, max_value, range)
      SELECT
        group_id,
        first_value,
        second_value,
        MIN(first_value) AS final_min_value,
        MAX(second_value) AS final_max_value,
        MAX(second_value) - MIN(first_value) AS range
      FROM
        AggregatedValues
      GROUP BY
        group_id, first_value, second_value
      ORDER BY
        group_id;
    `);
  } finally {
...
app.all('/calculate', async (req, res) => {
  ...
  if (selectedOption === '1') {
    ...
    `);
     ...
     // Calculate PPL
     const pplResult = await pool.query('SELECT ((xbar - lsl) / (3 * sdw))::numeric::decimal(10, 2) AS ppl FROM spc_result2');
     const ppl = pplResult.rows[0].ppl;
    ...
    const cplResult = await pool.query(`
      SELECT ((SELECT xbar FROM spc_result2 LIMIT 1) - 475) / (3 * (SELECT sdw FROM range_chart2 LIMIT 1))::numeric::decimal(10, 2) AS result
    `);
    const cpl = cplResult.rows[0].result;
    ...
    const uclResult = await pool.query(`
      SELECT
        ...
        ((SELECT A2 FROM standardsample WHERE sample_size = 2) * (SELECT rbar FROM range_chart2 LIMIT 1))::numeric::decimal(10, 2) AS result
    `);
    ...
    await pool.query('UPDATE spc_result2 SET ucl = $1, lcl = $2', [ucl, lcl]);
        ...
        res.json({ xbar, sd , pp , ppu , ppl , ppk , rbar , sdw , cp , cpu ,cpl , cpk,ucl,lcl});
          ...
          // Update Ppk in the spc_result table
          await pool.query('UPDATE spc_result SET Ppk = $1', [ppk]); 
// Calculate RBAR excluding last row if its range is 0
const rangeResult3 = await pool.query(`
    SELECT (SUM(range) / COUNT(range))::numeric::decimal(10, 2) AS result
    FROM range_chart3
    WHERE id != (SELECT MAX(id) FROM range_chart3 WHERE range = 0)
`);

const rbar = rangeResult3.rows[0].result;

// Update RBAR in spc_result3 table
await pool.query('UPDATE spc_result3 SET RBAR = $1', [rbar]);

// Update RBAR in range_chart3 table
